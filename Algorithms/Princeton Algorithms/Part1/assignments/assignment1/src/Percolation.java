...
public class Percolation {
	...
	private boolean[] mBottomConnected;
	...
	public Percolation(int N) {
		...
		mBottomConnected= new boolean[size];
	}
	...
	public void open(int i, int j) {
		if (checkIndex(i, j)) {
			...
			if (i != 1) {
				...
				if (i == mGridsize) {
					mBottomConnected[mPerc1.find(arrIndex)] = true;
				} 
			} else {
			...
			if (i != mGridsize) {
				if (mGridOpen[arrIndex + mGridsize]) {
					if (mBottomConnected[mPerc1.find(arrIndex + mGridsize )]) {
						mBottomConnected[mPerc1.find(arrIndex)] = true;
					} else if (mBottomConnected[mPerc1.find(arrIndex)]) {
						mBottomConnected[mPerc1.find(arrIndex + mGridsize )] = true;
					}
					mPerc1.union(arrIndex, arrIndex + mGridsize);
				}
			} 
			...
			if (j != mGridsize && mGridOpen[arrIndex + 1]) {
				...
				if (mBottomConnected[mPerc1.find(arrIndex + 1)]) {
					mBottomConnected[mPerc1.find(arrIndex)] = true;
				} else if (mBottomConnected[mPerc1.find(arrIndex)]) {
					mBottomConnected[mPerc1.find(arrIndex + 1)] = true;
				}
	...
	public boolean percolates() {
		return mBottomConnected[mPerc1.find(0)];
	}
